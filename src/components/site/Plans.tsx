...
import { Check, Globe, Monitor, Target, Building2, Package, Sparkles } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 relative">
        ...
        <div className="flex flex-col items-center gap-3 mb-10 sm:mb-14">
          <div className="inline-flex items-center gap-1 p-1 rounded-full bg-card border border-border">
            ...
            >
              {t("plans.yearly")}
              <span className="absolute -top-2 -right-2 px-1.5 py-0.5 rounded-full bg-emerald-500 text-white text-[9px] font-black uppercase tracking-wider">
                -{Math.round(YEARLY_DISCOUNT * 100)}%
              </span>
            </button>
            <Link
              to="/pacotes"
              className="relative px-4 py-2 rounded-full text-sm font-bold text-muted-foreground hover:text-foreground transition inline-flex items-center gap-1.5"
            >
              <Package className="w-3.5 h-3.5" />
              Pagamento único
              <span className="absolute -top-2 -right-2 px-1.5 py-0.5 rounded-full bg-amber-400 text-black text-[9px] font-black uppercase tracking-wider">
                Novo
              </span>
            </Link>
          </div>
          <p className="text-xs text-muted-foreground text-center max-w-md">
            ...
            {" · "}
            <Link to="/pacotes" className="text-primary font-semibold hover:underline inline-flex items-center gap-1">
              <Sparkles className="w-3 h-3" /> Prefere pagar uma vez só? Veja os pacotes prépagos
            </Link>
          </p>
        </div>
